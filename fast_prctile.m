function y = fast_prctile(x,p,dim)
%fast_prctile - Percentiles of a sample (fast version of builtin prctile function for scalar p)
%   Y = fast_prctile(X,P) returns percentiles of the values in X.  P is a scalar
...
%   dimension.
%
%   Y = fast_prctile(X,P,DIM) calculates percentiles along dimension DIM.  The
%   DIM'th dimension of Y has length LENGTH(P).
%
%   Percentiles are specified using percentages, from 0 to 100.  For an N
%   element vector X, fast_prctile computes percentiles as follows:
...
%
%   fast_prctile treats NaNs as missing values, and removes them.
%
%   Examples:
%      y = fast_prctile(x,50); % the median of x
%      y = fast_prctile(x,[2.5 25 50 75 97.5]); % a useful summary of x
%
...
if isempty(x)
    ...
    x = sort(x,1);  %Yair: sort() moves all NaN values to bottom of each column
    nonnans = ~isnan(x);
...
%         elseif isscalar(p)  %Yair
%             
%             %  % nearest value, non-interpolated
%             idx = n*p/100;
%             if double(uint16(idx)) == idx %MJP
%                 y = x(round(n*p/100),:);   %MJP
%             else  %MJP
%                 idxs = [fix(idx), ceil(idx)];
%                 y = interp1(idxs,x(idxs,:),idx);  % interpolated value
%             end  %MJP
        else
        ...
        for j = 1:ncols
            ...
            if nj > 0
                ...
                elseif isscalar(p)  %Yair
                    %y(:,j) = x(round(nj*p/100),j);  % nearest value, non-interpolated
                    idx = nj*p/100;
                    idxs = [fix(idx), ceil(idx)];
                    y(:,j) = interp1(idxs,x(idxs,j),idx);  % interpolated value
                else
